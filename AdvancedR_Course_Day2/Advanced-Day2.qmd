...
Okay, so convergence diagnostics look good. Let's take a look at the structural model assumptions. We'll use randomised quantile residuals. 
...
```{r}
qresid.sf <- cbind(select(df.sf, -c(spp1:spp5)), qresids.df)
qtm(pivot_longer(qresid.sf, cols = spp1:spp5, names_to = 'spp', values_to = 'abundance'), dots.col = 'abundance', by = 'spp')
```
