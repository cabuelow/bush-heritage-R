...
Here we build upon modelling skills developed in [Part 1](insert%20link) of this course. In **Part 1** we learned how to fit models to data in a *frequentist* paradigm.
...
![](Images/frequentists_vs_bayesians.png)

Find more [xkcd comics here](https://xkcd.com/1132/)
...
Now we can fit the model. We'll use [{Hmsc}](https://www.helsinki.fi/en/researchgroups/statistical-ecology/software/hmsc) to do this - flexible Bayesian modelling framework for fitting complex spatial models with random effects. 'Hmsc' is an acronym for 'Hierarchical modelling of species communities'. We can use it to fit univariate models (as we are right now) and multivariate models (as we'll do later).
...
Residuals <- scale(df$Species_abundance - Fitted.mean) # calculate standarised residuals
...
qtm(df.sf, dots.col = 'Fire_severity', dots.palette = 'Reds') +
qtm(pivot_longer(df.sf, cols = spp1:spp5, names_to = 'spp', values_to = 'abundance'), dots.col = 'abundance', by = 'spp')
```
...
Residuals <- scale(df[,3:7] - Fitted.mean) # calculate standarised residuals
par(mfrow = c(5,2)) # set graphical parameters
for(i in 1:ncol(Residuals)){
Fitted.mean.spp <- Fitted.mean[,i]
Residuals.spp <- Residuals[,i]
plot(Fitted.mean.spp, Residuals.spp, main = i); abline(a = 0, b = 0) # plot the fitted vs. residuals
qqnorm(Residuals.spp, main = i); qqline(Residuals.spp) # qq plot
}
```
...
```{r}
resid.sf <- cbind(select(df.sf, -c(spp1:spp5)), Residuals)
qtm(pivot_longer(resid.sf, cols = spp1:spp5, names_to = 'spp', values_to = 'abundance'), dots.col = 'abundance', by = 'spp')
```
...
TODO: need to expalin this beter
...
```{r}
spp <- c('spp1', 'spp2', 'spp3', 'spp4', 'spp5')
...
```{r}
library(fields)
xy <- data.frame(xyFromCell(grid_fire, 1:ncell(grid_fire)))
tps <- Tps(xy, values(grid_fire))
grid_fire_i <- interpolate(rast(grid_fire), tps)
qtm(grid_fire_i)
```

Now make predictions of species abundance using the grid of interpolated fire severity.

## Resources

-   Spatial R resources - Gecomputation with R
