...
Here we build upon modelling skills developed in **Part 1** of this course. In **Part 1**, we learned how to fit models to data in a *Frequentist* paradigm.
...
Remember, these predictions are in log-space, so we need to back-transform them to the natural scale (the inverse of the natural log is the exponent). But, as you can see, they are almost exactly the same as our frequentist model!

```{r}
exp(summary(mpost$Beta)$statistics[1]) # back transform the y-intercept (mean species abundance when fire severity = 0)
exp(summary(mpost$Beta)$statistics[2]) # back transform the beta coefficient (mean species abundance when fire severity = 0)
```
...
ns <- 5
b0 <- log(rep(10,ns))
b1 <- log(c(2,1.1,1,1.1,2))*c(-1,-1,0,1,1)
beta <- cbind(b0,b1)
X <- cbind(rep(1,n),runif(n, min = 0, max = 1)) # fire severity measurements
xycoords <- data.frame(x = runif(n, 152.1,152.5), y = runif(n, 28.1, 28.5)*-1)
sigma.spatial <- c(2)
alpha.spatial <- c(0.35)
Sigma <- sigma.spatial^2*exp(-as.matrix(dist(xycoords))/alpha.spatial)
eta1 <- mvrnorm(mu=rep(0,n), Sigma=Sigma)
lambda1 <- c(1,2,-2,-1,0)
spatial_res <- eta1%*%t(lambda1)
lambda <- exp((X%*%t(beta))) + spatial_res

y <- data.frame(apply(data.frame(lambda), 2, function(x) rpois(n, x)))
colnames(y) <- c('spp1', 'spp2', 'spp3', 'spp4', 'spp5')
df <- data.frame(xycoords, y, Fire_severity = X[,2])
head(df)
```
...
qtm(pivot_longer(df.sf, cols = spp1:spp5, names_to = 'spp', values_to = 'abundance'), dots.col = 'abundance', dots.size = 0.5) + tm_facets(by = 'spp', ncol = 2, free.scales = T)
```
...
We still see some spatial clustering of residuals - not very different from before. But atleast now we know that the patterns aren't due to spatial autocorrelation, and our other diagnostic plots look much better.
...
And plot the residual correlations between species. These could be because species interact with one another (e.g., predator-prey interactions), we're missing some important explanatory variables in our model, and/or Tobler's first law (i.e., spatial autocorrelation).
...
```{r}
tmap_mode('view')
qtm(df.sf, dots.col = 'Fire_severity', dots.size = 0.5)
```
...
Any questions, get in touch with Christina [here](cabuelow.github.io/personal-website/)