...
Here we build upon modelling skills developed in [**Part 1**](Statistical-modelling-Part_1.html) of this course. In **Part 1**, we learned how to fit models to data in a *Frequentist* paradigm.
...
Here is a great overview of the key differences between [Frequentist and Bayesian parameter estimation](https://towardsdatascience.com/maximum-likelihood-vs-bayesian-estimation-dd2eb4dfda8a).
...
Here's a quick example of how we can turn our interactive maps into something that is publication ready.

```{r}
tmap_mode('plot')
tm_shape(stack) + # Specify the shapefile or raster to map
  ...
  tm_dots( # Now we are displaying points (dots)
    title = '\nFire severity', # Title as it appears in the legend
  ...
  tm_layout(panel.labels = c('Sp.1', 'Sp.2', 'Sp.3', 'Sp.4', 'Sp.5'), # Define labels
            legend.position = c(-0.5, 0), # Customise legend
            legend.title.fontface = 'bold') +
  tm_compass(position = c(0.01, 0.85)) + # Add a north arrow
