...
Here we build upon modelling skills developed in [**Part 1**](Statistical-modelling-Part_1.html) of this course. In **Part 1**, we learned how to fit models to data using *Frequentist* statistics.
...
**Download the models you'll need for today from [here](models-functions.zip)**
...
Diving into the fundamental and nuanced differences between **Frequentist** and **Bayesian** statistics is beyond the scope of this short course. But you can find some nice explanations of the key differences [here](https://towardsdatascience.com/statistics-are-you-bayesian-or-frequentist-4943f953f21b) and [here](https://medium.com/@roshmitadey/frequentist-v-s-bayesian-statistics-24b959c96880).
...
In [**Part 1**](Statistical-modelling-Part_1.html) of this course we touched on how model parameters (coefficients) are estimated with either ordinary least squares (general linear models) or maximum likelihood (generalised linear models).
...
Here is a brief overview of the key differences between [Frequentist and Bayesian parameter estimation](https://towardsdatascience.com/maximum-likelihood-vs-bayesian-estimation-dd2eb4dfda8a).
...
In the interest of time, we'll just repeat the generalised linear model of species abundance in relation to fire severity that we did in [**Part 1**](Statistical-modelling-Part_1.html). But if you're keen, feel free to try the general linear model of tree circumference (you'll just need to specify a normal (gaussian) distribution instead of a poisson).

First, let's simulate the data we need for species abundance and fire severity (repeating exactly as we did in [**Part 1**](Statistical-modelling-Part_1.html)).
...
Now we can fit the model. We'll use [{Hmsc}](https://www.helsinki.fi/en/researchgroups/statistical-ecology/software/hmsc) to do this - a flexible Bayesian modelling framework for fitting complex hierarchical models with spatial or non-spatial random effects. 'Hmsc' is an acronym for 'Hierarchical modelling of species communities'. We can use it to fit univariate models (as we are right now) and multivariate models (as we'll do later).
...
Unfortunately our handy `plot` function we used in [**Part 1**](Statistical-modelling-Part_1.html) won't work for {Hmsc} model objects. So we have to calculate the model residuals ourselves and plot them.
...
Both look about the same (so high dispersion in the count data isn't an issue for interpreting the residuals). And looks very similar to the diagnostic plots we made in [**Part 1**](Statistical-modelling-Part_1.html), where we fitted a Frequentist GLM to the same data.
...
Remember, these predictions are in log-space, so we need to back-transform them to the natural scale (the inverse of the natural log is the exponent). But, as you can see, they are almost exactly the same as our Frequentist model from [**Part 1**](Statistical-modelling-Part_1.html)!
...
Species 5 looks okay, but there are some patterns in the residuals vs. fitted plots for species 1-4. What's going on here? Let's see if we can improve our model by including a spatial random effect to account for spatial autocorrelation.
...
Better than the previous model with a non-spatial random effect.
...
Now, let's see how our coefficient estimates compare to **known truths**.
...
The coefficients are a bit biased from the *known truths*, even though we've accounted for spatial autocorrelation. This illustrates an important point regarding interpretation of beta coefficients now that we have included a random effect in the model. The beta coefficients are  *conditional* on the random effect. This means that, the y-intercepts (b0 coefficients) are the average predicted species abundance when Fire severity = 0, *conditional* on the random effect (spatial autocorrelation). Similarly, the slopes (b1 coefficients) are the average predicted change in species abundance in response to a one unit increase in fire severity, *conditional* on the random effect. 

Let's remind ourselves what those conditional expectations look like. We plotted this earlier.
...
So, we would in fact expect the *conditional* y-intercept for species 3 to be on average higher than the other species. Similarly, we would expect the y-intercept for species 2 to be on average lower than the other. So our model has done a good job of capturing this.

We would also expect that, *conditional* on spatial autocorrelation, the abundance of species 3 to decrease slightly with increasing Fire severity - similar to species 2. The model has captured this too. 

Finally, now that we've accounted for dependencies in our observations due to spatial autocorrelation, we can be confident that we have adequately represented uncertainty in the coefficient estimates. Check out [this paper](https://www.nature.com/articles/s41586-024-07236-z) for a nice example of what happens when we don't properly account for lack of independence of observations.
...
So, after accounting for variability in species abundance due to fire severity, we infer that abundances of species 1 are positively correlated with abundances of species 2, and negatively correlated with species 3 and 4. This is exactly what we expected, given how we simulated their reponses to the spatial latent variable.
...
LVs <- c('Latent.variable.1', 'Latent.variable.2', 'Latent.variable.3', 'Latent.variable.4', 'Latent.variable.5')
...
Now that we've removed variability in species abundances due to fire severity, we can see species composition and abundance is longer are associated fire severity.
...
```{r}
XData_new <- data.frame(values(grid_fire_i))
colnames(XData_new) <- 'Fire_severity'
```
