...
So the coefficients are still biased from the *known truth* even though we've accounted for spatial autocorrelation. This is likely due to the random variation in the data, and so estimates might improve if we up the sample size. But now that we've accounted for dependencies in our observations due to spatial autocorrelation, we can be confident that we have adequately represented uncertainty in the coefficient estimates. Check out [this paper](https://www.nature.com/articles/s41586-024-07236-z) for a nice example of what happens when we don't properly account for lack of independence of observations. 

We can also check whether the model has recovered the scale of spatial autocorrelation that we simulated in the data. Let's remind ourselves of what that was:
...
Looks good. We can also get a summary for alpha providing the mean estimate.

```{r}
summary(mpost$Alpha[[1]][,1])
```

So again, the mean estimate is slightly biased from 0.35, our *known truth*, but could improve with more data.
