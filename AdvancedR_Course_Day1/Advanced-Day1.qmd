...
We would expect this if our response variable ($y$) is continuous and unbounded (could potentially be any number between $-∞$ and $∞$). In the next section, we'll learn how to fit **generalised** linear models which allow us to model non-normal error distributions necessary for count data, for example.
...
In addition to how much variability in the data our model is explaining, we also want to want to make sure the model isn't violating any structural assumptions, namely that the residuals are:
...
As we've just seen, data simulation is a powerful tool for developing models and enhancing our understanding of how they work. Let's try fitting some to real data.
TODO
...
$$log(\lambda_i) = b_0 + b_{1}X_{1i},$$
*Remember $\lambda$ is the mean species abundance (and also the variance).* And $\lambda$ is what we need to simulate counts of abundance from a pooisson distribution,
...
$$\lambda_i = exp(b_0 + b_{1}X_{1i}).$$
We can then use these predicted values of $\lambda$ to simulate counts of species abundance from a poisson distribution. Let's simulate counts this way below.
...
summary(m)
```
Right away, we can see that the beta coefficient is are biased from the **known truth**. Now check the diagnostics.
...
Not great, but we didn't expect it to be. Let's try a **generalised linear model** with a poisson family (error distribution) and a log link function instead.
...
To check our model diagnostics, we'll use a function from {boot} that will display deviance residuals for the GLM (as opposed to standard residuals we saw before). [Learn more here](https://www.datascienceblog.net/post/machine-learning/interpreting_generalized_linear_models/).
...
Here we have learned how to fit models in a frequentist paradigm, using either ordinary least squares or maximum likelihood. As a follow-up [here](insert link to day 2), we will learn how to fit models in a Bayesian paradigm to allow us to fit more complex multivariate models.