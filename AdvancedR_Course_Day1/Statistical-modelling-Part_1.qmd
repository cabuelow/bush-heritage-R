...
*Any questions about these course notes, contact the [author here](https://cabuelow.github.io/personal-website/)*
...
-   $y$ is our response variable of interest,
...
-   $b_1$ is the coefficient representing the strength and direction of the relationship between $y$ and $X_{1}$ (the **slope**) - which can be interpreted as the average change in $y$ in response to a one unit shift in $X$, and
...
In the above example we only have one explanatory variable ($X_{1}$), but we can have multiple (referred to as 'multiple linear regression'). We would just add on a new term to our equation for every additional explanatory variable, e.g. $b_{2}X_{2i}$. If we add other explanatory variables, then the interpretation of their beta coefficients (slopes) becomes 'the average change in $y$ in response to a one unit shift in $X$, *holding all other variables constant*'. This is the beauty and power of multiple linear regression, we can evaluate the effect of each explantory variable in isolation from others.
...
We would expect this if our response variable ($y$) is continuous and unbounded (could potentially be any number between $-∞$ and $∞$). (Note: In the next section, we'll learn how to fit **generalised** linear models which allow us to model non-normal error distributions necessary for count data, for example.)
...
**Tip** Try increasing the standard deviation of the natural variation we've simulated (e_sd) and see how that changes the R^2^.
...
Let's try coding Fire as character string instead of zeros and ones and see what happens and why we need to change `characters` to `factors` when modelling. We'll use three categories instead of 2 because its more interesting

```{r}
set.seed(123)
# simulation parameters
b0 <- 5 # average tree circumference at Un-burned locations (i.e, y-intercept)
b1 <- -0.5 # beta coefficient representing the average difference in tree circumference at 'sort of' Burned locations compared to Un-burned
b2 <- -1 # beta coefficient representing the average difference in tree circumference at Burned locations compared to Un-burned
e_mu <- 0 # mean error
e_sd <- 0.1 # error standard deviation
n <- 100 # number of tree circumference measurements

# simulation variables
X <- c(rep(c('Unburned', 'SortOfBurned', 'Burned'), n/3), 'Burned') # un-burned (0) vs. burned (1) locations
e <- rnorm(n, e_mu, e_sd) # natural error

# dummy code
X1 <- ifelse(X == 'SortOfBurned', 1, 0)
X2 <- ifelse(X == 'Burned', 1, 0)

# simulate y
y <- b0 + b1*X1 + b2*X2 + e # simulate tree circumference observations at unburned vs. burned locations
df <- data.frame(Fire_severity = X, Tree_circumference = y) # make a dataframe of observations
str(df)
```
...
Why did `ggplot` switch the order around. It uses alphabetical order for character strings. So let's turn Fire into an factor and set the reference level to be 'Unburned'.
...
```{r}
df <- df %>% 
  mutate(Fire_severity = factor(Fire_severity, levels = c('Unburned', 'SortOfBurned', 'Burned')))
str(df)
```
...
Much better. Let's try a model on this data.
...
We can use the poisson distribution to simulate data that mimics counts (e.g. species abundance). Mathematically, this looks like: $$y_i \sim Poisson(\lambda_i).$$ *Remember* that $\lambda$ represents the expected mean species abundance (and also the variance in species' abundance).

So how can we use our linear model $y_i = b_0 + b_{1}X_{1i}$ to estimate expected mean species abundance ($\lambda$) for each value of our explanatory variable ($X$) so that we can simulate observed species abundance $y$ from a poisson distribution? Before, in our *general* linear model that assumed normally-distributed errors, we just added on a normally distributed error term $\epsilon$ to our linear equation. That won't work for us anymore because counts are discrete values greater than 0.

So we can *generalize* our linear equation so that it provides values that are constrained between 0 and $∞$ (instead of $-∞$ and $∞$). We use something called the **link function** to do this.

All *generalised* linear models have a link function that *links* the mean of $y$ - which in this case is $\lambda$ because we are using a poisson distribution to model counts - to our linear equation of explanatory variables. The canonical link function for the poisson distribution is the `log link`. We can write this mathematically as: $$log(\lambda_i) = b_0 + b_{1}X_{1i}$$ The `log link` will ensure that all of the predictions from our linear equation are greater than 0. In practice, to estimate $\lambda$, we apply the inverse of the link function to both sides of the equation. The inverse of the natural log is the exponent. Doing this looks like: $$\lambda_i = exp(b_0 + b_{1}X_{1i}).$$

Once we have calculated expected species abundance for all values of the explanatory variable ($\lambda$), we can use this to simulate observed species abundance drawn randomly from a poisson distribution, which is what we wanted to do initially: $$y_i \sim Poisson(\lambda_i).$$

Let's give it a try. Before we do though, one more thing to note. By taking the exponent of the linear combination of explanatory variable(s), we're imposing a non-additive, multiplicative relationship between $X$ and $y$. This changes how we interpret the $b_{1}$ coefficient. Instead of representing the additive change in $y$ for every one unit increase in $X$, $b_{1}$ now represents the multiplicative change.
...
So the general linear model is on average making biased predictions (the median residual value is -0.4). But the coefficient estimates aren't that bad - it's estimated average species abundance when Fire severity is 0 to be ~10 (the y-intercept) and the slope to be ~-5, meaning that with a one unit increase in Fire severity, species abundance will decrease by 5 units. 

That is pretty close to what we simulated - a 2 factor decrease in species abundance with a one unit increase in fire severity.

```{r}
exp(b0+b1) # expected species abundance with a one unit increase in Fire severity
```
...
Again, not too bad given we know this is a mis-specified model. So what's the problem with using this model? Well, imagine what would happen if Fire severity increased by 3 units instead of 1, what would the model predict for species abundance?

```{r}
10 + (-5*4) # predicting species abundance with a 3 unit increase in Fire severity with the additive, general linear model
```

It has predicted negative species abundance, which of course we know is not possible. 

So let's try a **generalised linear model** with a poisson family (error distribution) and a log link function instead. Remember the purpose of the log link function is to ensure predictions are equal to or greater than zero, and also means that we interpret this as a multiplicative model (instead of additive).
...
We can also check the diagnostic plots. If we see a **'fan-shape'** in the residuals vs. fitted plot, implying errors are greater when fitted (predicted) abundance is higher, we can suspect that our data is over-dispersed for the model we are trying to fit to it.

Let's simulate over-dispersed species abundance and see for ourselves. We'll keep everything as before, except we will use lambda to simulate from a negative binomial distribution instead of the poisson and set theta, the shape parameter, to 0.8.
